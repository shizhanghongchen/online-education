...
/**
  * @description: dws service
  * @Author: my.yang
  * @Date: 2019/8/30 8:17 PM
  */
object DwsMemberService {
  ...
  def importMemberUseApi(sparkSession: SparkSession, dt: String) = {
    ...
    // 导入广播Join sortMergeJoin ---> hashJoin
    // 使用窗口函数也需要先导包
    ...
    // 3. 内存优化 DF / DS cache
    //    .as[DwsMember].persist(StorageLevel.MEMORY_AND_DISK_SER)
    //    result.show()
    // 1. 内存优化 Rdd普通缓存
    //      .rdd.cache()
    // 2. 内存优化 Kyro序列化
    //      .rdd.persist(StorageLevel.MEMORY_AND_DISK_SER)
    //    result.foreach(item => println(item.adname))
